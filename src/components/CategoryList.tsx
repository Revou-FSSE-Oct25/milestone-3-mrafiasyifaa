...
const CategoryList = ({ selectedCategory, onTabSelect }: Props) => {
  ...
  const allCategories = [{ id: 0, name: 'All', slug: 'all', image: '' }, ...categories]

  return (
    <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex item-center gap-3 text-sm font-poppins">
            {loading ? (
              <>
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="border border-revoshop-accent/30 px-4 py-1.5 md:px-6 rounded-md bg-gray-200 animate-pulse w-20 h-9" />
                ))}
              </>
            ) : (
